...
# get config address from fs.yml
configHost=$(bash scripts/parseYaml.sh configs/fs.yml '.app.host')
if [ "${configHost}" = "127.0.0.1" ]; then
  configHost="localhost"
fi
configPort=$(bash scripts/parseYaml.sh configs/fs.yml '.http.port')
configAddr="${configHost}:${configPort}"
enableMode=$(bash scripts/parseYaml.sh configs/fs.yml '.http.tls.enableMode')
schemes="http"
if [ "$enableMode" != "" ]; then
  schemes="https"
fi
...
swaggerAddr=$(grep -E '^[[:space:]]*//[[:space:]]*@host' "cmd/fs/main.go" | awk '{print $3}')
if [[ -z "${swaggerAddr}" ]]; then
  swaggerAddr="localhost:8080"
fi
if [ "${configAddr}" != "${swaggerAddr}" ];then
  sed -i "s/${swaggerAddr}/${configAddr}/g" cmd/fs/main.go
fi

# generate api docs
swag init -g cmd/fs/main.go
checkResult $?
