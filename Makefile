...
graph:
	...
	@cp -f cmd/fs/main.go .
	go-callvis -skipbrowser -format=svg -nostd -file=fs fs
	@rm -f main.go fs.gv
...
.PHONY: build
# Build fs for linux amd64 binary
build:
	@echo "building 'fs', linux binary file will output to 'cmd/fs'"
	@cd cmd/fs && CGO_ENABLED=0 GOOS=linux GOARCH=amd64 go build
