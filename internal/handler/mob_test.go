...
func newMobHandler() *gotest.Handler {
	testData := &model.Mob{}
	testData.ID = 1
	...
	d.IDao = dao.NewMobDao(d.DB, c.ICache.(cache.MobCache))

	// init mock handler
	h := gotest.NewHandler(d, testData)
	h.IHandler = &mobHandler{iDao: d.IDao.(dao.MobDao)}
	iHandler := h.IHandler.(MobHandler)
...
func Test_mobHandler_Create(t *testing.T) {
	h := newMobHandler()
	defer h.Close()
	testData := &types.CreateMobRequest{}
	_ = copier.Copy(testData, h.TestData.(*model.Mob))
...
func Test_mobHandler_DeleteByID(t *testing.T) {
	h := newMobHandler()
	defer h.Close()
	testData := h.TestData.(*model.Mob)
...
func Test_mobHandler_UpdateByID(t *testing.T) {
	h := newMobHandler()
	defer h.Close()
	testData := &types.UpdateMobByIDRequest{}
	_ = copier.Copy(testData, h.TestData.(*model.Mob))
...
func Test_mobHandler_GetByID(t *testing.T) {
	h := newMobHandler()
	defer h.Close()
	testData := h.TestData.(*model.Mob)
...
func Test_mobHandler_List(t *testing.T) {
	h := newMobHandler()
	defer h.Close()
	testData := h.TestData.(*model.Mob)
	...
	result := &httpcli.StdResult{}
	err := httpcli.Post(result, h.GetRequestURL("List"), &types.ListMobsRequest{query.Params{
		Page:  0,
		Limit: 10,
...
func TestNewMobHandler(t *testing.T) {
	defer func() {
		recover()
	}()
	_ = NewMobHandler()
