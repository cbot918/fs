...
func newItemHandler() *gotest.Handler {
	testData := &model.Item{}
	testData.ID = 1
	...
	d.IDao = dao.NewItemDao(d.DB, c.ICache.(cache.ItemCache))

	// init mock handler
	h := gotest.NewHandler(d, testData)
	h.IHandler = &itemHandler{iDao: d.IDao.(dao.ItemDao)}
	iHandler := h.IHandler.(ItemHandler)
...
func Test_itemHandler_Create(t *testing.T) {
	h := newItemHandler()
	defer h.Close()
	testData := &types.CreateItemRequest{}
	_ = copier.Copy(testData, h.TestData.(*model.Item))
...
func Test_itemHandler_DeleteByID(t *testing.T) {
	h := newItemHandler()
	defer h.Close()
	testData := h.TestData.(*model.Item)
...
func Test_itemHandler_UpdateByID(t *testing.T) {
	h := newItemHandler()
	defer h.Close()
	testData := &types.UpdateItemByIDRequest{}
	_ = copier.Copy(testData, h.TestData.(*model.Item))
...
func Test_itemHandler_GetByID(t *testing.T) {
	h := newItemHandler()
	defer h.Close()
	testData := h.TestData.(*model.Item)
...
func Test_itemHandler_List(t *testing.T) {
	h := newItemHandler()
	defer h.Close()
	testData := h.TestData.(*model.Item)
	...
	result := &httpcli.StdResult{}
	err := httpcli.Post(result, h.GetRequestURL("List"), &types.ListItemsRequest{query.Params{
		Page:  0,
		Limit: 10,
